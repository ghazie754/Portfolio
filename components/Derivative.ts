...
function Operator(input:Input) {
  ...
  this.solve = (segment1: { coefficient: any; type: string; }, segment2: { coefficient: any; type: string; }, x: any) => {
...
function MathFunction(input:Input) {
  this.f = input;
  if (!input) {
    XCalc.log("Math function has no input.");
  }

  this.solve = (segment: { coefficient: any; }) => {
...
const Segment: new (input: any) => {
  ...
  this.removeBrackets = (value: string) => {
    if (!value) return "";
  ...
  const findLast = (operator: string, value: string) => {
      ...
      for (var i = 0; i < value.length; i++) {
        ...
        if (i == index) {
          ...
          if (
            (openBrackets === 0 &&
              (operator != "-" ||
                (i > 0 && operators.indexOf(value.substr(i - 1, 1)) == -1) ||
                i === 0)) ||
            (openBrackets == 1 && operator == "(")
          ) {
            inBrackets = false;
            break;
            ...
            if (operator != "^") {
              ...
              if (nextOperator == -1) {
                ...
                index =
                  index + 1 + value.substring(index + 1).indexOf(operator);
              }
            }
          }
        }
  ...
  const findLastTrig = (trig: string, value: string)=> {
    let matches:RegExpExecArray | null| number = 0;
    let index = -1;
    let r:RegExp | number = 0;
    if (trig == "sin") {
      r = /(a)?sin/g;
    } else if (trig == "cos") {
      r = /(a)?cos/g;
    ...
    for (matches = r.exec(value); matches; matches = r.exec(value))
      if (RegExp.$1 != "a") index = matches.index;
    var inBrackets = true;
      ...
      for (var i = 0; i < value.length; i++) {
        ...
        if (i == index) {
          ...
          if (openBrackets === 0) {
            ...
            index = -1;
            for (matches = r.exec(sub); matches; matches = r.exec(sub))
              if (RegExp.$1 != "a") index = matches.index;
          }
        }
      }
  ...
  const findMultiplicationBrackets = function (value: string) {
    //Keep searching for the next last sign if the one found is within brackets
    let inBracketsOpen = true;
    let inBracketsClosed = true;
    let indexOpen = -1;
    let indexClosed = -1;
    let operators = "+-/*^sincostanabsloglnsqrt";
      ...
      for (var i = 0; i < value.length; i++) {
        ...
        if (i == indexOpen && inBracketsOpen) {
          if (
            openBrackets == 1 &&
            i !== 0 &&
            operators.indexOf(value.substr(i - 1, 1)) == -1
          ) {
  ...
  const equals = function (expression: { type: any; mathFunction: { f: any; }; sections: any[]; variable: any; coefficient: any; operator: { operator: any; }; }) {
    if (this.type != expression.type) {
      return false;
    } else {
      if (this.type == "function") {
        return (
          this.mathFunction.f == expression.mathFunction.f &&
          this.sections[0].equals(expression.sections[0])
        );
        ...
        if (this.operator.operator == "*" || this.operator.operator == "+") {
          return (
            this.operator.operator == expression.operator.operator &&
            ((this.sections[0].equals(expression.sections[0]) &&
              this.sections[1].equals(expression.sections[1])) ||
              (this.sections[0].equals(expression.sections[1]) &&
                this.sections[1].equals(expression.sections[0])))
          );
        } else {
          return (
            this.operator.operator == expression.operator.operator &&
  ...
  const solve = function (x: number) {
    ...
    if (this.type == "value") {
      ...
      if (this.sections.length == 1) {
        ...
        return this.operator.solve(
          this.sections[0].solve(x),
          this.sections[1].solve(x),
          x
        );
      }
    }
    ...
    if (expression.containsX() && typeof expression != "variable") {
      ...
      if (expression.type == "section") {
        if (expression.operator.operator == "+") {
          if (
            expression.sections[0].type == "value" &&
            expression.sections[0].coefficient === 0
          ) {
            expression = expression.sections[1];
          } else if (
            expression.sections[1].type == "value" &&
            expression.sections[1].coefficient === 0
          ) {
          ...
          if (
            expression.sections[0].type == "value" &&
            expression.sections[0].coefficient == 1
          ) {
            expression = new Segment(expression.sections[1]);
          } else if (
            expression.sections[1].type == "value" &&
            expression.sections[1].coefficient == 1
          ) {
            ...
            expression.sections[1].type == "section" &&
            expression.sections[1].operator.operator == "/" &&
            expression.sections[1].sections[0].type == "value" &&
            expression.sections[1].sections[0].coefficient == 1
          ) {
            expression.operator = new Operator("/");
            expression.sections[1] = expression.sections[1].sections[1];
          } else if (
            expression.sections[0].type == "section" &&
            expression.sections[0].operator.operator == "/" &&
            expression.sections[0].sections[0].type == "value" &&
            expression.sections[0].sections[0].coefficient == 1
          ) {
            expression.operator = new Operator("/");
            ...
            expression.sections[1].type == "section" &&
            expression.sections[1].operator.operator == "^" &&
            expression.sections[0].sections[0].equals(
              expression.sections[1].sections[0]
            )
          ) {
  ...
  const derivative = function () {
    ...
    if (expression.type == "variable") {
      ...
      if (expression.type == "section" && expression.operator.operator == "^") {
        ...
        if (
          expression.sections[1].containsX() &&
          expression.sections[0].containsX()
        ) {
          s1 = expression;
 ...
 const prettyFormula = function () {
    var str =
      "<div class='group " +
      this.type +
      (this.type == "section" && this.operator.operator == "/"
        ? " division"
        : "") +
      "'>";
    ...
    if (this.type == "value") {
      ...
      if (
        this.sections[0].type == "section" &&
        this.sections[0].operator.operator != "/"
      ) {
      ...
      if (this.operator.operator == "^") {
        str +=
          "<div class='group exponent'>" +
          this.sections[1].prettyFormula() +
          "</div>";
      } else if (this.operator.operator == "/") {
        str +=
          "<div class='group denom'>" +
          this.sections[1].prettyFormula() +
          "</div>";
      } else {
        if (
          this.sections[1].type == "section" &&
          this.sections[0].operator.operator != "/"
        ) {
...
if ( input !== undefined) {
    if (typeof input == "string") {
      ...
      if (xMultiplication2 > xMultiplication)
        xMultiplication = xMultiplication2;
      var functionMultiplication = -1;
      var operators = "+-/*^sincostanabsloglnsqrt";
      if (
        sin > multiplication &&
        (sin === 0 || operators.indexOf(input.substr(sin - 1, 1)) == -1)
      )
        functionMultiplication = sin;
      if (
        cos > multiplication &&
        (cos === 0 || operators.indexOf(input.substr(cos - 1, 1)) == -1)
      )
        functionMultiplication = cos;
      if (
        tan > multiplication &&
        (tan === 0 || operators.indexOf(input.substr(tan - 1, 1)) == -1)
      )
        functionMultiplication = tan;
      if (
        asin > multiplication &&
        (asin === 0 || operators.indexOf(input.substr(asin - 1, 1)) == -1)
      )
        functionMultiplication = asin;
      if (
        acos > multiplication &&
        (acos === 0 || operators.indexOf(input.substr(acos - 1, 1)) == -1)
      )
        functionMultiplication = acos;
      if (
        atan > multiplication &&
        (atan === 0 || operators.indexOf(input.substr(atan - 1, 1)) == -1)
      )
        functionMultiplication = atan;
      if (
        abs > multiplication &&
        (abs === 0 || operators.indexOf(input.substr(abs - 1, 1)) == -1)
      )
        functionMultiplication = abs;
      if (
        log > multiplication &&
        (log === 0 || operators.indexOf(input.substr(log - 1, 1)) == -1)
      )
        functionMultiplication = log;
      if (
        ln > multiplication &&
        (ln === 0 || operators.indexOf(input.substr(ln - 1, 1)) == -1)
      )
        functionMultiplication = ln;
      if (
        sqrt > multiplication &&
        (sqrt === 0 || operators.indexOf(input.substr(sqrt - 1, 1)) == -1)
      )
        functionMultiplication = sqrt;
      if (
        xMultiplication > multiplication &&
        (xMultiplication === 0 ||
          operators.indexOf(input.substr(xMultiplication - 1, 1)) == -1)
      )
        functionMultiplication = xMultiplication;
      ...
      if (addition != -1 && addition > subtraction) {
        ...
        this.sections.push(
          new Segment(input.substring(0, functionMultiplication))
        );
        this.sections.push(
          new Segment(input.substring(functionMultiplication))
        );
        this.operator = new Operator("*");
      } else if (
        multiplication2 != -1 &&
        Math.max(multiplication2, multiplication, division) == multiplication2
      ) {
        ...
        sin != -1 &&
        Math.max(sin, cos, tan, asin, acos, atan, abs, log, ln, sqrt) == sin
      ) {
        ...
        cos != -1 &&
        Math.max(cos, tan, asin, acos, atan, abs, log, ln, sqrt) == cos
      ) {
        ...
        tan != -1 &&
        Math.max(tan, asin, acos, atan, abs, log, ln, sqrt) == tan
      ) {
        ...
        asin != -1 &&
        Math.max(asin, acos, atan, abs, log, ln, sqrt) == asin
      ) {
        ...
        acos != -1 &&
        Math.max(acos, atan, abs, log, ln, sqrt) == acos
      ) {
        ...
        if (openBrackets === 0) {
          var bracket2 = i - 1;
          if (bracket1 > 0)
            this.sections.push(new Segment(input.substring(0, bracket1)));
          if (bracket2 - bracket1 != 1)
            this.sections.push(
              new Segment(input.substring(bracket1 + 1, bracket2))
            );
          if (bracket2 != input.length - 1)
            this.sections.push(new Segment(input.substring(bracket2 + 1)));
          this.operator = new Operator("*");
        } else {
...
function Graph(value: any, width: number, height: number, startx1: undefined, startx2: undefined, starty1: string | undefined, starty2: undefined) {
  ...
  this.update = function () {
    ...
    for (var i = x1; i <= x2; i += accuracy) {
      ...
      if (
        (points[points.length - 1].y !== 0 && !points[points.length - 1].y) ||
        Math.abs(points[points.length - 1].y) > 10000
      ) {
  ...
  var drawAxes = function (_x1: number, _x2: number, _y1: number, _y2: number) {
    ...
    if (0 >= _x1 - 30 && 0 <= _x2 + 30) {
      ...
      stage.beginPath();
      stage.moveTo(
        canvas.width / 2 - ((_x2 + _x1) / 2 / (_x2 - _x1)) * canvas.width,
        0
      );
      stage.lineTo(
        canvas.width / 2 - ((_x2 + _x1) / 2 / (_x2 - _x1)) * canvas.width,
      ...
      for (
        i = 0;
        i <= limit;
        i += Math.pow(10, Math.floor(Math.log(_y2 - _y1) / Math.LN10)) / 2
      ) {
        if (i === 0) continue;
        if (i <= _y2 + 50) {
          stage.beginPath();
          stage.moveTo(
            canvas.width / 2 -
              ((_x2 + _x1) / 2 / (_x2 - _x1)) * canvas.width -
              5,
            canvas.height - ((i - _y1) / (_y2 - _y1)) * canvas.height
          );
          stage.lineTo(
            canvas.width / 2 -
              ((_x2 + _x1) / 2 / (_x2 - _x1)) * canvas.width +
    ...
    if (0 >= _y1 - 50 && 0 <= _y2 + 50) {
      ...
      stage.moveTo(
        0,
        canvas.height / 2 + ((_y2 + _y1) / 2 / (_y2 - _y1)) * canvas.height
      );
      stage.lineTo(
        canvas.width,
        canvas.height / 2 + ((_y2 + _y1) / 2 / (_y2 - _y1)) * canvas.height
      ...
      for (
        i = 0;
        i <= limit;
        i += Math.pow(10, Math.floor(Math.log(_x2 - _x1) / Math.LN10)) / 2
      ) {
        if (i === 0) continue;
        if (i <= _x2 + 50) {
          stage.beginPath();
          stage.moveTo(
            ((i - _x1) / (_x2 - _x1)) * canvas.width,
            canvas.height / 2 +
              ((_y2 + _y1) / 2 / (_y2 - _y1)) * canvas.height -
              5
          );
          stage.lineTo(
            ((i - _x1) / (_x2 - _x1)) * canvas.width,
  ...
  this.redraw = function () {
    if (points.length > 1) {
      ...
      while (
        ...
        i++;
      graphStage.moveTo(
        (i / points.length) * canvas.width,
        canvas.height - ((points[i].y + offsetY) / (y2 - y1)) * canvas.height
      );
      for (i++; i < points.length; i++) {
        if (
          Math.abs(
            canvas.height -
              ((points[i].y + offsetY) / (y2 - y1)) * canvas.height -
              (canvas.height -
                ((points[i - 1].y + offsetY) / (y2 - y1)) * canvas.height)
          ) <= canvas.height &&
          points[i].y !== undefined &&
          Math.abs(points[i].y) != Infinity &&
          !isNaN(points[i].y)
          ...
          graphStage.lineTo(
            (i / (points.length - 1)) * canvas.width,
            canvas.height -
              ((points[i].y + offsetY) / (y2 - y1)) * canvas.height
          );
        }
        if (
          ...
          graphStage.moveTo(
            (i / (points.length - 1)) * canvas.width,
            canvas.height - ((0 + offsetY) / (y2 - y1)) * canvas.height
          );
        }
      }
  ...
  this.setRange = function (_x1: any, _x2: any, _y1: string | undefined, _y2: any) {
    x1 = _x1;
    x2 = _x2;
    y1 = _y1;
    y2 = _y2;
  ...
  var getMousePos = function (evt: { clientX: number; clientY: number; }) {
  ...
  //Starts panning
  var startDrag = function (event: any) {
  ...
  var dragMouse = function (event: { preventDefault: () => void; }) {
    stage.fillStyle = "#FFFFFF";
    stage.fillRect(0, 0, canvas.width, canvas.height);
    ...
    if (
      Math.abs(mousePos.x - startMouse.x) > canvas.width * 0.2 ||
      Math.abs(mousePos.y - startMouse.y) > canvas.height * 0.2
    ) {
      redrawLine();
      ...
      stage.drawImage(
        graphCanvas,
        mousePos.x - startMouse.x,
        mousePos.y - startMouse.y
      );
    }
  ...
  var endDrag = function (event: any) {
  ...
  var moveMouse = function (event: any) {
    if (distX === 0 && distY === 0) {
      ...
      if (
        points[Math.round((mousePos.x / canvas.width) * points.length)].y ===
          0 ||
        points[Math.round((mousePos.x / canvas.width) * points.length)].y
      ) {
        //Draw the coordinate
        ...
        stage.arc(
          mousePos.x,
          canvas.height -
            ((points[Math.round((mousePos.x / canvas.width) * points.length)]
              .y +
              offsetY) /
              (y2 - y1)) *
              canvas.height,
        ...
        if (mousePos.x < stage.measureText(txt).width / 2 + 2) {
          ...
          mousePos.x >
          canvas.width - stage.measureText(txt).width / 2 - 2
        ) {
          stage.textAlign = "right";
        } else {
          stage.textAlign = "center";
  ...
  var endMouseOver = function (event: any) {
    canvas.removeEventListener("mousemove", moveMouse, false);
  ...
  var scrollZoom = function (event: { wheelDelta: any; detail: number; preventDefault: () => void; }) {
    ...
    stage.drawImage(
      graphCanvas,
      (canvas.width * (1 - (x2 - x1) / (x2 - 2 * distX - x1))) / 2,
      (canvas.height * (1 - (y2 - y1) / (y2 - 2 * distY - y1))) / 2,
      canvas.width * ((x2 - x1) / (x2 - 2 * distX - x1)),
      canvas.height * ((y2 - y1) / (y2 - 2 * distY - y1))
    );
    if (event.preventDefault) event.preventDefault();
    clearTimeout(timer);
...
const XCalc = (function () {
  var worker = <any>{};

  worker.errors = [];

  //logs errors
  worker.log = function (message: any) {
  ...
  worker.properBrackets = function (value: string) {
    var openBrackets = 0;
    for (var i = 0; i < value.length; i++) {
      if (value.substr(i, 1) == "(") openBrackets++;
      if (value.substr(i, 1) == ")") openBrackets--;
  ...
  worker.createExpression = function (value: any) {
  ...
  worker.graphExpression = function (value: any, width: any, height: any, x1: any, x2: any, y1: any, y2: any) {
    if (this.properBrackets(value)) {
...
function simplifyText(event: undefined) {
  ...
  var y1 = document.getElementById("y1").value
    ? document.getElementById("y1").value.trim() == "auto"
      ? "auto"
      : parseFloat(document.getElementById("y1").value)
    : undefined;
  var y2 = document.getElementById("y2").value
    ? document.getElementById("y2").value.trim() == "auto"
      ? "auto"
  ...
  var timer = setTimeout(function () {
    var width = 500;
    if (document.getElementById("wrapper").offsetWidth < 550)
      width = document.getElementById("wrapper").offsetWidth - 50;
    ...
    if (derive) {
      ...
      derivativeGraph = XCalc.graphExpression(
        derivative,
        width,
...
function onKeyUp(event: { keyCode: number; }) {
  if (event.keyCode == 13) {
    simplifyText();
  }
}
